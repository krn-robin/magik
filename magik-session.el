...
(defcustom magik-session-sentinel-hooks nil
  "*Hooks to run after the magik session process has finished.
...
(defvar magik-session-exec-path nil
  "Stored value of variable `exec-path' when the Magik session process was started.")
(make-variable-buffer-local 'magik-session-exec-path)

(defvar magik-session-process-environment nil
  "Stored value of `process-environment' when the Magik session process was started.")
...
Important for M-p and M-n commands.  The first command typed is number 0.
...
(defun magik-session-shell ()
  "Start a command shell with the same environment as the current Magik session process."
  (interactive)
  (require 'shell)
...
Commands are sent to the session with \\[magik-session-newline] or
...
process.  This will default to a unique currently unused name based upon
...
;;
;;; Each command is recorded by magik-session-prev-cmds so that the
