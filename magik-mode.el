...
(easy-menu-define magik-menu magik-base-mode-map
  ...
  `(,"Magik"
    ...
    [,"Mark Method"       magik-mark-method               :active t :keys "C-M-h"]
    [,"Copy Method"       magik-copy-method               :active t :keys "C-c C-e c"]
...
(defun magik-expand-abbrev ()
  "Expansion hook for Magik keyword abbrevs.
Prefix the just-expanded word with an underscore when it stands as a
complete token (not preceded by `.'), and re-indent the current line
when the keyword closes or continues a block construct (`_else',
`_elif', `_finally', `_using', `_with', `_when', `_protection',
`_end')."
  (save-excursion
    (let*
...
With a nil ARG, toggle the current setting.  With a positive numeric
ARG, enable transmission; with a non-positive ARG, disable it."
  (interactive "P")
...
The rule is that the thing must start against the left margin."
  ...
  (save-excursion
    (let ((original-point (point))
          (stack nil))
      (when-let* ((beg (re-search-backward "^\\w" nil t)))
        (forward-line -1)
        (while (and (not (bobp))
            ...
            (cond
             ...
             ((assoc (car tok) magik-ends-and-begins)
              (if (equal (cdr (assoc (car stack) magik-begins-and-ends)) (car tok))
                  (pop stack)
                (goto-char (cdr tok))
                (user-error "Found '%s' when expecting '%s'"
                            (car tok)
                            (cdr (assoc (car stack) magik-begins-and-ends)))))))
          (forward-line))
        (if (< (point) original-point)
            (user-error "Don't know what to transmit")
...
Optional argument BUFFER-B ..."
  ...
  (let (reg-A-beg reg-A-end reg-B-beg reg-B-end)
    (setq buffer-A (or buffer-A (current-buffer)))
    (with-current-buffer buffer-A
      (magik-mark-method)
      (setq reg-A-beg (region-beginning)
            reg-A-end (region-end)))
    (setq buffer-B (or buffer-B (progn (other-window 1) (current-buffer))))
...
;; `magik-mark-method-exchange-mode' does not need a setup call here:
;; its behaviour is controlled directly by `magik-mark-method-exchange'.
...
(progn
  ...
  (define-key magik-base-mode-map "\C-\M-h" 'magik-mark-method) ;standard mark-defun key
  ...
  ;; Sending to session — follows python/ruby/CIDER conventions
  (define-key magik-base-mode-map (kbd "C-c C-c") 'magik-transmit-method)
  (define-key magik-base-mode-map (kbd "C-c C-r") 'magik-transmit-region)
  (define-key magik-base-mode-map (kbd "C-c C-b") 'magik-transmit-buffer)
  (define-key magik-base-mode-map (kbd "C-c C-l") 'magik-transmit-$-chunk)
  (define-key magik-base-mode-map (kbd "C-c RET") 'magik-transmit-thing)
  (define-key magik-base-mode-map (kbd "C-c C-z") 'magik-session)

  ;; Pragma and documentation
  (define-key magik-base-mode-map (kbd "C-c ;")   'magik-single-pragma)
  (define-key magik-base-mode-map (kbd "C-c :")   'magik-file-pragma)
  (define-key magik-base-mode-map (kbd "C-c C-;") 'magik-single-method-sw-method-doc)
  (define-key magik-base-mode-map (kbd "C-c C-:") 'magik-file-sw-method-doc)

  ;; Class Browser / documentation lookup — C-c C-d prefix
  (define-key magik-base-mode-map (kbd "C-c C-d d") 'magik-cb)
  (define-key magik-base-mode-map (kbd "C-c C-d c") 'magik-cb-paste-class)
  (define-key magik-base-mode-map (kbd "C-c C-d m") 'magik-cb-paste-method)
  (define-key magik-base-mode-map (kbd "C-c C-d b") 'magik-cb-paste-method-and-class)
  (define-key magik-base-mode-map (kbd "C-c C-d j") 'magik-cb-jump-to-source)
  (define-key magik-base-mode-map (kbd "C-c C-d /") 'magik-cb-and-clear)

  ;; Editing / work buffer — C-c C-e prefix
  (define-key magik-base-mode-map (kbd "C-c C-e c") 'magik-copy-method)
  (define-key magik-base-mode-map (kbd "C-c C-e e") 'magik-ediff-methods)
  (define-key magik-base-mode-map (kbd "C-c C-e d") 'magik-cb-magik-ediff-methods)
  (define-key magik-base-mode-map (kbd "C-c C-e m") 'magik-copy-method-to-buffer)
  (define-key magik-base-mode-map (kbd "C-c C-e n") 'magik-set-work-buffer-name)
  (define-key magik-base-mode-map (kbd "C-c C-e r") 'magik-copy-region-to-buffer)
  (define-key magik-base-mode-map (kbd "C-c C-e s") 'magik-add-debug-statement)
  (define-key magik-base-mode-map (kbd "C-c C-e w") 'magik-compare-methods)

  ;; Miscellaneous
  (define-key magik-base-mode-map (kbd "C-c C-t") 'magik-trace-curr-statement)
  (define-key magik-base-mode-map (kbd "C-c C-q") 'magik-fill-public-comment)
  (define-key magik-base-mode-map (kbd "C-c C-p") 'magik-version-selection)
  (define-key magik-base-mode-map (kbd "C-c C-i") 'magik-completion-invalidate-cache))
